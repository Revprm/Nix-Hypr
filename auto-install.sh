...
# --- Modified Section ---
# Using the local copy.sh script to install all dotfiles from the assets folder
printf "$NOTE Installing all configs from the assets folder...\n"
if [ -f ./copy.sh ]; then
    chmod +x ./copy.sh
    ./copy.sh
else
    echo -e "$ERROR copy.sh not found in the current directory. Skipping config installation."
fi
